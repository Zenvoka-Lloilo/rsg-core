...
function RSGCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.SetJobObject(jobObject)
        -- Accepts a full job object for custom jobs (e.g., society jobs)
        if not jobObject or type(jobObject) ~= 'table' then return false end
        if not jobObject.name or not jobObject.label then return false end

        self.PlayerData.job = jobObject

        if not self.Offline then
            -- Sync to player statebag for client-side updates
            Player(self.PlayerData.source).state:set('job', jobObject, true)

            -- Update player data (sends to client)
            self.Functions.UpdatePlayerData()

            -- Trigger job update events
            TriggerEvent('RSGCore:Server:OnJobUpdate', self.PlayerData.source, self.PlayerData.job)
            TriggerClientEvent('RSGCore:Client:OnJobUpdate', self.PlayerData.source, self.PlayerData.job)
